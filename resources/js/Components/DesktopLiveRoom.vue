<script setup>
/*
 * DesktopLiveRoom — Masaüstü tam-ekran canlı yayın deneyimi.
 * MobileLiveRoom.vue ile İŞLEV olarak birebir aynı (feed + sohbet + teklif + onay + sabit bilgi),
 * fakat GÖRSEL yerleşim masaüstüne uygun: SOLDA geniş video, SAĞINDA dikey sohbet/teklif paneli.
 * - Bağlantıyı KENDİ KURMAZ: Show.vue'daki paylaşılan LiveKit odasını (:room) kullanır.
 *   Tam ekran açma/kapama yalnızca görsel bir geçiş → reconnect yok, "bağlanılıyor" çıkmaz.
 */
...
function pushFeed(item) {
    ...
    if (feed.value.length > 80) feed.value.splice(0, feed.value.length - 80);
    nextTick(() => {
...
onMounted(async () => {
    ...
    // Güvenlik: 'playing'/'loadeddata' hiç gelmezse yine de fade-in yap → siyah kalmasın
...
<template>
    <div class="dlr" data-testid="desktop-live-room">
        <div class="dlr-stage">
            <video ref="videoEl" class="dlr-video" :class="{ ready: videoReady }" autoplay playsinline muted
                   @click="toggleMute" @loadeddata="videoReady = true" @playing="videoReady = true" data-testid="dlr-video"></video>

            <div v-if="connState === 'checking'" class="dlr-state" data-testid="dlr-checking">
                <div class="dlr-spin"></div>
                <p>Yayına bağlanılıyor…</p>
            </div>
            <div v-else-if="connState === 'offline'" class="dlr-state" data-testid="dlr-offline">
                <i class="bi bi-camera-video-off"></i>
                <p>Satıcı henüz yayın başlatmadı</p>
            </div>

            <div class="dlr-stage-top">
                <div class="dlr-stage-left">
                    <img class="dlr-ava" :src="a.seller?.profile_img" :alt="a.seller?.name">
                    <div class="dlr-seller">
                        <div class="dlr-seller-name">{{ a.seller?.name }}</div>
                        <div class="dlr-viewers"><i class="bi bi-people-fill"></i> {{ viewers }} izleyici</div>
                    </div>
                    <span v-if="connState === 'live'" class="dlr-live" data-testid="dlr-live-pill"><span class="dot"></span> CANLI</span>
                </div>
                <button class="dlr-icon" @click="toggleMute" :title="muted ? 'Sesi aç' : 'Sesi kapat'" data-testid="dlr-mute">
        ...
        <div class="dlr-side">
            <div class="dlr-side-head">
                <div class="dlr-title"><i class="bi bi-broadcast"></i> Canlı Yayın</div>
                <button class="dlr-close" @click="close" title="Kapat (Esc)" data-testid="dlr-close">
                    <i class="bi bi-x-lg"></i>
                </button>
            </div>

            <div class="dlr-price">
                <div class="dlr-price-item">
                    <span class="lbl">Güncel</span>
                    <span class="val" data-testid="dlr-top-price">{{ topPrice }}</span>
                </div>
                <div class="dlr-price-item">
                    <span class="lbl"><i class="bi bi-clock"></i> Kalan</span>
                    <span class="val" :class="{ crit: remaining > 0 && remaining <= 60 }" data-testid="dlr-remaining">{{ remainingText }}</span>
                </div>
                <div class="dlr-price-item">
            ...
            <div ref="feedBox" class="dlr-feed" data-testid="dlr-feed">
                <div v-if="!feed.length" class="dlr-feed-empty">
                    <i class="bi bi-chat-dots"></i>
                    <span>Henüz mesaj yok. İlk mesajı sen yaz veya teklif ver!</span>
                </div>
                <div v-for="it in feed" :key="it._k"
                     class="dlr-row" :class="it.kind === 'bid' ? 'is-bid' : 'is-chat'"
                     :data-testid="it.kind === 'bid' ? 'dlr-feed-bid' : 'dlr-feed-chat'">
            ...
            <div class="dlr-bottom">
                <template v-if="isAuth && a.is_active && !a.is_owner">
                    <div v-if="pendingBid" class="dlr-confirm" data-testid="dlr-bid-confirm">
                        <span><b>{{ fmtTL(pendingBid) }}</b> teklif verilsin mi?</span>
                        <div class="dlr-confirm-btns">
                            <button class="dlr-btn ghost" @click="cancelBid" data-testid="dlr-bid-cancel">Vazgeç</button>
                            <button class="dlr-btn go" :disabled="sending" @click="confirmBidNow" data-testid="dlr-bid-confirm-yes">
                                <i class="bi bi-lightning-charge-fill"></i> Onayla
                            </button>
                        </div>
                    </div>

                    <div v-if="mode === 'bid' && !pendingBid" class="dlr-chips" data-testid="dlr-quick-chips">
                        <button v-for="(q, i) in quickBids" :key="i" class="dlr-chip" @click="quickPick(q.amount)" :data-testid="`dlr-quick-${i}`">{{ q.label }}</button>
                    </div>

                    <div v-if="errorMsg" class="dlr-err" data-testid="dlr-error">{{ errorMsg }}</div>

                    <div v-if="!pendingBid" class="dlr-input-row">
                        <div class="dlr-toggle" data-testid="dlr-mode-toggle">
                            <button :class="{ on: mode === 'chat' }" @click="mode = 'chat'; errorMsg=''" data-testid="dlr-mode-chat">Sohbet</button>
                            <button :class="{ on: mode === 'bid' }" @click="mode = 'bid'; errorMsg=''" data-testid="dlr-mode-bid">Teklif</button>
                        </div>
                        <input v-if="mode === 'chat'" v-model="draft" class="dlr-input" type="text" maxlength="300"
                               placeholder="Mesaj yaz…" @keyup.enter="onSubmit" data-testid="dlr-chat-input">
                        <input v-else v-model="bidDraft" class="dlr-input" type="number" :min="minBid" :step="step"
                               :placeholder="`En az ${fmtTL(minBid)}`" @keyup.enter="onSubmit" data-testid="dlr-bid-input">
                        <button class="dlr-send" @click="onSubmit" :disabled="sending" data-testid="dlr-send">
                            <i class="bi" :class="mode === 'bid' ? 'bi-lightning-charge-fill' : 'bi-send-fill'"></i>
                        </button>
                    </div>
                </template>
                <template v-else-if="a.is_owner">
                    <div class="dlr-note">Bu sizin ilanınız — yayın yönetimi için satıcı panelini kullanın.</div>
                </template>
                <template v-else-if="!isAuth">
                    <a :href="config.login_url" class="dlr-login" data-testid="dlr-login">Sohbet ve teklif için giriş yap</a>
                </template>
                <template v-else>
                    <div class="dlr-note">Bu müzayede için teklif kapalı.</div>
                </template>
            </div>
        </div>
    </div>
